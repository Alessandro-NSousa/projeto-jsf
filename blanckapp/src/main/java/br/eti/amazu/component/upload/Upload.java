...
public class Upload implements Serializable {
	 ...
	 * Largura da imagem em pixels (se o arquivo for uma imagem). actionComplete =
	 * uma acao a ser realizada apos o upload. updateActionComplete = qual
	 * componente sofrerah update apos o actionComplete.
	 */
	public Upload(DialogBean dialogBean, // .O dialogBean jah vem inicializado
		...
		this.init(dialogBean, uploadMode, writherPath, targetFile, imageHeight, imageWidth, actionComplete, update,
				multiplo);
	}

	public Upload(DialogBean dialogBean, UploadMode uploadMode, String whriterPath, String actionComplete,
			Boolean multiplo) {

		this.init(dialogBean, uploadMode, whriterPath, null, null, null, actionComplete, null, multiplo);
	}

	void init(DialogBean dialogBean, UploadMode uploadMode, String writherPath, String targetFile, Integer imageHeight,
			Integer imageWidth, String actionComplete, String update, Boolean multiplo) {
		...
		try {
			...
			if (multiplo == false) {
				if (targetFile != null) {
					if (!file.getFileName().equals(targetFile)) {
						...
						dialogBean.addMessage(
								this.getMessage("MAR001", new String[] { file.getFileName(), targetFile }),
								DialogType.ERROR);
				...
				if (!msgs.isEmpty()) {
					...
					dialogBean.addMessage(this.getMessage("MAR012", new String[] { file.getFileName() }),
					...
					if (uploadMode.equals(UploadMode.FILE_SERVER)) {
						...
						dialogBean.addActionMessage(this.getMessage("MAR010", new String[] { file.getFileName() }),
								actionComplete, update);
			...
			dialogBean.addMessage(this.getMessage("MAR011", new String[] { file.getFileName() }), DialogType.ERROR);
	 ...
	 * get/set
	 ---------*/
	public String getWritherPath() {
		return writherPath;
	}

	public void setWritherPath(String writherPath) {
		this.writherPath = writherPath;
	}
